...
- 과도하게 깊거나 큰 트리, 또는 input 같은 인터랙티브 태그가 들어오면 patch 전에 위험 입력으로 판단해 안전하게 차단하는지
- 형제 순서만 변경된 reorder 상황에서 index 기반 diff의 불안정성을 감지하고 patch를 강행하지 않고 전체 render로 복구하는지
